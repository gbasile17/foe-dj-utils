...
The collection itself adds another layer of work outside the app. Lossless DJ-oriented libraries tend to grow heterogeneously over time — FLACs from one source, AIFFs from another, MP3s ripped years ago — and they pick up the kinds of metadata problems Music.app does nothing to fix: artist tags with "101. " number prefixes from track-listing exports, missing or wrong genres, near-duplicate files that differ only in encoding or filename, and the inevitable format conversions where naive tools strip your tags.

This command-line utility aims to solve both sides of that. It provides a set of focused tools for working *around* Music.app rather than fighting it — detecting and recreating desynced playlists, quarantining tracks whose cloud status blocks sync, separating lossless local files from cloud-synced copies — and a unified set of audio-file utilities for the DJ collection itself: a single `audiotag` interface that reads and writes MP3 / FLAC / WAV / AIFF tags so format conversions (FLAC → AIFF via `ffmpeg`) preserve every field, hash-and-title duplicate detection across arbitrary directories, MusicBrainz-backed genre tagging, and the usual title/artist search and tag cleanup helpers.
